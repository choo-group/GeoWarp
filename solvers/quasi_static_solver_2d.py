...
print("=" * 72)
print("  PROGRAM INFO")
print("    Module             :: Implicit MPM - Quasi-static solver (2D)")
...
class quasi_static_solver_2d:
	def __init__(self,
		...
		self.increment_solution = wp.zeros(shape=self.n_matrix_size, dtype=wp.float64, requires_grad=True)
		self.increment_iteration = wp.zeros(shape=self.n_matrix_size, dtype=wp.float64)

		self.rows = wp.zeros(shape=2*25*self.n_matrix_size+self.n_matrix_size, dtype=wp.int32)
		self.cols = wp.zeros(shape=2*25*self.n_matrix_size+self.n_matrix_size, dtype=wp.int32)
		self.vals = wp.zeros(shape=2*25*self.n_matrix_size+self.n_matrix_size, dtype=wp.float64)
		...
		self.selector_x_list, self.selector_y_list, self.e_x_list, self.e_y_list = precompute_seed_vectors_2d(self.n_grid_x, self.n_grid_y, self.n_nodes, self.n_matrix_size, self.max_selector_length)

	# ============ Class functions ============
	def reset(self):
		self.increment_solution.zero_()
		...
		for iter_id in range(self.n_iter):
			...
			for c_iter in range(5):
				for r_iter in range(5):
					...
					jacobian_wp = tape.gradients[self.increment_solution]
					wp.launch(kernel=from_jacobian_to_vector_parallel_2d,
			...
			wp.launch(kernel=set_diagnal_component_for_boundary_and_deactivated_dofs_2d,
			...
			wp.launch(kernel=from_increment_to_solution,
					  dim=self.n_matrix_size,
					  inputs=[self.increment_iteration, self.increment_solution])
	...
	def advance_one_step(self, current_step, total_steps):
		...
		# P2G
		wp.launch(kernel=P2G_2d,
				  dim=self.n_particles,
				  inputs=[self.x_particles, self.inv_dx, self.dx, self.n_grid_x, self.n_nodes, self.dofStruct.activate_flag_array, self.GIMP_lp])

		# Newton iteration
		self.newton_iter(current_step, total_steps)

		# G2P
		wp.launch(kernel=G2P_2d,
				  dim=self.n_particles,
				  inputs=[self.GIMP_lp, self.x_particles, self.dx, self.inv_dx, self.n_grid_x, self.n_nodes, self.increment_solution, self.deformation_gradient_total_old, self.deformation_gradient_total_new, self.left_Cauchy_Green_old, self.left_Cauchy_Green_new])
