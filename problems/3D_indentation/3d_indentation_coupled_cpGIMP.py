...
for training_iter in range(n_training_steps):
	# ============ Load steps ============
	n_steps = 25
	...
	# Inverse
	print('Loss after finishing load steps:', solver.loss.numpy(), 'Force:', solver.indentation_force.numpy()[0], 'E_grad:', solver.E_grad)

	# Gradient descent
	updated_E = youngs_modulus - learning_rate * solver.E_grad[0]
	print('Updated E:', updated_E)
	youngs_modulus = updated_E

	# reset everything
	solver.reinit_everything()

	# reset grid
	dx = max_x/n_grid_x # 0.25 m
	dy = max_y/n_grid_y
	dz = max_z/n_grid_z
	solver.reset_grid(dx, dy, dz)

	# reset elastic modulus
	solver.reset_youngs_modulus(youngs_modulus)
