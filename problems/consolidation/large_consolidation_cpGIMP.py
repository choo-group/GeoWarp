...
# Implicit MPM solver for coupled poromechanics (large consolidation)
...
def P2G(x_particles: wp.array(dtype=wp.vec2d),
    ...
    for i in range(0, i_range):
        for j in range(0, j_range):
            ...
            if weight>wp.float64(1e-6): # THIS SEEMS TO BE CRITICAL
...
# mobility function following Eq. (86) in Uzuoka & Borja IJNAMG 2012
@wp.func
def mobility(new_J: wp.float64,
             lame_lambda: wp.float64,
             lame_mu: wp.float64,
             mobility_initial: wp.float64
             ):
    mobility_new = mobility_initial * ((wp.float64(2.)*lame_mu + lame_lambda) * new_J) / (lame_mu*(wp.float64(1.)+wp.float64(1.)/wp.pow(new_J, wp.float64(2.))) + (wp.float64(1.)-wp.log(new_J))*lame_lambda/wp.pow(new_J, wp.float64(2.)))
    return mobility_new
...
def get_GIMP_shape_function_and_gradient_avg(xp: wp.vec2d,
    ...
    # Averaged GIMP function referring to Coombs et al. CMAME 2018
...
def assemble_residual_GIMP(x_particles: wp.array(dtype=wp.vec2d),
    ...
    mobility_initial = wp.float64(1e-3)
    mobility = mobility(particle_J, lame_lambda, lame_mu, mobility_initial)
    ...
    if top_boundary_flag[p]==True:
        traction_y = wp.float64(-10.0) * dx/PPD #* (step+wp.float64(1.0))/wp.float64(100.0)
...
n_matrix_size = 3 * n_nodes # 3 includes the spatial dimension (2) and pore water pressure
...
# Find the top left particle
top_left_particle_id = np.argwhere((particle_pos_np[:,0]<start_x+dx/PPD) & (particle_pos_np[:,1]>end_y-dy/PPD))[0][0]
...
wp.launch(kernel=initialization,
          dim=n_particles,
          inputs=[deformation_gradient_total_new, deformation_gradient_total_old, left_Cauchy_Green_new, left_Cauchy_Green_old, x_particles, top_boundary_flag, end_x, end_y, dx, PPD])
...
for step in range(120):
    ...
    wp.launch(kernel=G2P_GIMP, #G2P,
              dim=n_particles,
              inputs=[x_particles, deformation_gradient_total_new, deformation_gradient_total_old, left_Cauchy_Green_new, left_Cauchy_Green_old, particle_Cauchy_stress_array, pressure_array, inv_dx, dx, inv_dy, dy, lame_lambda, lame_mu, n_grid_x, n_nodes, new_solution, old_solution, dofStruct, GIMP_lp])
    ...
    x_numpy = np.array(x_particles.numpy())
    output_particles = meshio.Mesh(points=x_numpy, cells=[], point_data={'stress_yy': particle_Cauchy_stress_array.numpy()[:,1,1], 'pressure': pressure_array.numpy()})
    output_particles.write("terzaghi_particles_%d.vtk" % (step+1))
    ...
    # Save top displacement and time
    top_disp_time_array = np.append(top_disp_time_array, np.array([x_numpy[top_left_particle_id][1]-particle_pos_np[top_left_particle_id][1], total_time]))


    # Compress the background grid
    dy -= (x_beginning_numpy[top_left_particle_id][1] - x_numpy[top_left_particle_id][1])/100.
    inv_dy = 1.0/dy
...
position_pressure_np = np.column_stack([x_numpy[:,1]-start_y, pressure_array_np])
position_pressure_np = np.reshape(position_pressure_np, (-1, 2))

top_disp_time_array = np.reshape(top_disp_time_array, (-1, 2))

with np.printoptions(threshold=np.inf):
    print(position_pressure_np)

    print(top_disp_time_array)
