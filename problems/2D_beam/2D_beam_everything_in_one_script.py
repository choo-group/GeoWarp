...
def assemble_residual(x_particles: wp.array(dtype=wp.vec2d),
    ...
    if base_int[0]==0:
        w = wp.matrix(
            ...
            fx[1]-float64_one, wp.float64(0.5)*wp.pow(fx[1]-wp.float64(0.5), wp.float64(2.0)),
            shape=(3,2)
        )
    ...
    for iter_id in range(n_iter):
        ...
        with tape:
            ...
            wp.launch(kernel=assemble_residual,
                      dim=n_particles,
                      inputs=[x_particles, inv_dx, dx, rhs, new_solution, old_solution, p_vol, p_rho, lame_lambda, lame_mu, deformation_gradient, n_grid_x, n_nodes, dofStruct, step, particle_external_flag_array])
        ...
        with np.printoptions(threshold=np.inf):
            ...
            print('residual.norm:', np.linalg.norm(rhs.numpy()), 'solver state:', solver_state)
